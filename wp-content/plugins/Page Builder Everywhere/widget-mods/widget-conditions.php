...
class PBE_Widget_Conditions {
    ...
    public static function widget_conditions_options_echo( $major = '', $minor = '' ) {
        ...
        switch ( $major ) {
            case 'category':
                ?>
                <option value=""><?php _e( 'All category pages', 'PBE' ); ?></option>
                <?php
            ...
            case 'loggedin':
                ?>
                <option value="loggedin" <?php selected( 'loggedin', $minor ); ?>><?php _e( 'Logged In', 'PBE' ); ?></option>
                <option value="loggedout" <?php selected( 'loggedout', $minor ); ?>><?php _e( 'Logged Out', 'PBE' ); ?></option>
                <?php
            break;
            case 'author':
                ?>
                <option value=""><?php _e( 'All author pages', 'PBE' ); ?></option>
                <?php
            ...
            case 'tag':
                ?>
                <option value=""><?php _e( 'All tag pages', 'PBE' ); ?></option>
                <?php
            ...
            case 'date':
                ?>
                <option value="" <?php selected( '', $minor ); ?>><?php _e( 'All date archives', 'PBE' ); ?></option>
                <option value="day"<?php selected( 'day', $minor ); ?>><?php _e( 'Daily archives', 'PBE' ); ?></option>
                <option value="month"<?php selected( 'month', $minor ); ?>><?php _e( 'Monthly archives', 'PBE' ); ?></option>
                <option value="year"<?php selected( 'year', $minor ); ?>><?php _e( 'Yearly archives', 'PBE' ); ?></option>
                <?php
            break;
            case 'page':
                ...
                ?>
                <option value="front" <?php selected( 'front', $minor ); ?>><?php _e( 'Front page', 'PBE' ); ?></option>
                <option value="posts" <?php selected( 'posts', $minor ); ?>><?php _e( 'Posts page', 'PBE' ); ?></option>
                <option value="archive" <?php selected( 'archive', $minor ); ?>><?php _e( 'Archive page', 'PBE' ); ?></option>
                <option value="404" <?php selected( '404', $minor ); ?>><?php _e( '404 error page', 'PBE' ); ?></option>
                <option value="search" <?php selected( 'search', $minor ); ?>><?php _e( 'Search results', 'PBE' ); ?></option>
                <optgroup label="<?php esc_attr_e( 'Post type:', 'PBE' ); ?>">
                    <?php
                ...
                <optgroup label="<?php esc_attr_e( 'Static page:', 'PBE' ); ?>">
                    <?php
            ...
            case 'taxonomy':
                ?>
                <option value=""><?php _e( 'All taxonomy pages', 'PBE' ); ?></option>
                <?php
            ...
            case 'post_type':
                ?>
                <optgroup label="<?php echo esc_attr_x( 'Single post:', 'a heading for a list of custom post types', 'PBE' ); ?>">
                    <?php
                ...
                <optgroup label="<?php echo esc_attr_x( 'Archive page:', 'a heading for a list of custom post archive pages', 'PBE' ); ?>">
                    <?php
    ...
    public static function widget_conditions_has_children_echo( $major = '', $minor = '', $has_children = false ) {
        ...
        ?>
            <label>
                ...
                <?php echo esc_html_x( "Include children", 'Checkbox on Widget Visibility if choosen page has children.', 'PBE' ); ?>
            </label>
        <?php
        }
    }
    ...
    public static function widget_conditions_admin( $widget, $return, $instance ) {
        ...
        <div class="widget-conditional <?php if ( empty( $_POST['widget-conditions-visible'] ) || $_POST['widget-conditions-visible'] == '0' ) { ?>widget-conditional-hide<?php } ?>">
            ...
            <?php if ( ! isset( $_POST['widget-conditions-visible'] ) ) { ?><a href="#" class="button display-options"><?php _e( 'Where?', 'PBE' ); ?></a><?php } ?>
            <div class="widget-conditional-inner">
                <div class="condition-top">
                    <?php printf( _x( '%s if:', 'placeholder: dropdown menu to select widget visibility; hide if or show if', 'PBE' ), '<select name="conditions[action]"><option value="show" ' . selected( $conditions['action'], 'show', false ) . '>' . esc_html_x( 'Display', 'Used in the "%s if:" translation for the widget visibility dropdown', 'PBE' ) . '</option><option value="hide" ' . selected( $conditions['action'], 'hide', false ) . '>' . esc_html_x( 'Hide', 'Used in the "%s if:" translation for the widget visibility dropdown', 'PBE' ) . '</option></select>' ); ?>
                </div><!-- .condition-top -->
                        ...
                        <div class="condition">
                            <div class="selection alignleft">
                                <select class="conditions-rule-major" name="conditions[rules_major][]">
                                    <option value="" <?php selected( "", $rule['major'] ); ?>><?php echo esc_html_x( '-- Select --', 'Used as the default option in a dropdown list', 'PBE' ); ?></option>
                                    <option value="category" <?php selected( "category", $rule['major'] ); ?>><?php esc_html_e( 'Category', 'PBE' ); ?></option>
                                    <option value="author" <?php selected( "author", $rule['major'] ); ?>><?php echo esc_html_x( 'Author', 'Noun, as in: "The author of this post is..."', 'PBE' ); ?></option>

                                    <?php if( ! ( defined( 'IS_WPCOM' ) && IS_WPCOM ) ) { // this doesn't work on .com because of caching ?>
                                        <option value="loggedin" <?php selected( "loggedin", $rule['major'] ); ?>><?php echo esc_html_x( 'User', 'Noun', 'PBE' ); ?></option>
                                        <option value="role" <?php selected( "role", $rule['major'] ); ?>><?php echo esc_html_x( 'Role', 'Noun, as in: "The user role of that can access this widget is..."', 'PBE' ); ?></option>
                                    <?php } ?>

                                    <option value="tag" <?php selected( "tag", $rule['major'] ); ?>><?php echo esc_html_x( 'Tag', 'Noun, as in: "This post has one tag."', 'PBE' ); ?></option>
                                    <option value="date" <?php selected( "date", $rule['major'] ); ?>><?php echo esc_html_x( 'Date', 'Noun, as in: "This page is a date archive."', 'PBE' ); ?></option>
                                    <option value="page" <?php selected( "page", $rule['major'] ); ?>><?php echo esc_html_x( 'Page', 'Example: The user is looking at a page, not a post.', 'PBE' ); ?></option>
                                    <option value="post_type" <?php selected( "post_type", $rule['major'] ); ?>><?php echo esc_html_x( 'Post Type', 'Example: the user is viewing a custom post type archive.', 'PBE' ); ?></option>
                                    <?php if ( get_taxonomies( array( '_builtin' => false ) ) ) : ?>
                                        <option value="taxonomy" <?php selected( "taxonomy", $rule['major'] ); ?>><?php echo esc_html_x( 'Taxonomy', 'Noun, as in: "This post has one taxonomy."', 'PBE' ); ?></option>
                                    <?php endif; ?>
                                </select>

                                <?php _ex( 'is', 'Widget Visibility: {Rule Major [Page]} is {Rule Minor [Search results]}', 'PBE' ); ?>

                                <select class="conditions-rule-minor" name="conditions[rules_minor][]" <?php if ( ! $rule['major'] ) { ?> disabled="disabled"<?php } ?> data-loading-text="<?php esc_attr_e( 'Loading...', 'PBE' ); ?>">
                            ...
                            <div class="condition-control">
                                <span class="condition-conjunction"><?php echo esc_html_x( 'or', 'Shown between widget visibility conditions.', 'PBE' ); ?></span>
                                <div class="actions alignright">
                                    <a href="#" class="delete-condition"><?php esc_html_e( 'Delete', 'PBE' ); ?></a> | <a href="#" class="add-condition"><?php esc_html_e( 'Add', 'PBE' ); ?></a>
                                </div>
                            </div>
...
add_action( 'init', array( 'PBE_Widget_Conditions', 'init' ) );