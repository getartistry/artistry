...
class CPRO_iSDKException extends Exception
{
}
...
class CPRO_iSDK 
{
    ...
    public function cfgCon($name, $key = "", $dbOn = "on")
    {
        ...
        try {
            ...
            if (strpos($connected, 'ERROR') !== FALSE) {
                throw new CPRO_iSDKException($connected);
            }

        } catch (CPRO_iSDKException $e) {
            throw new CPRO_iSDKException($e->getMessage());
        }
    ...
    public function vendorCon($name, $user, $pass, $key = "", $dbOn = "on")
    {
        ...
        if ($key != "" && $key != "on" && $key != "off" && $key != 'kill' && $key != 'throw') {
            ...
            if (!empty($details[$name])) {
                if ($details[$name][2] == "i") {
                    ...
                    throw new CPRO_iSDKException("Invalid application name: \"" . $name . "\"");
                }
            } else {
                throw new CPRO_iSDKException("Application Does Not Exist: \"" . $name . "\"");
            }
            $this->key = $details[$name][3];
        }
        ...
        try {
            ...
            throw new CPRO_iSDKException("Connection Failed");
        }
        return TRUE;
    }
    ...
    public function methodCaller($service, $callArray)
    {
        ...
        if (!$result->faultCode()) {
            ...
            if ($this->debug == "kill") {
                ...
                throw new CPRO_iSDKException($result->faultString(), $result->faultCode());
    ...
    /**
     * @service Misc CPRO_iSDK Functions
     */
