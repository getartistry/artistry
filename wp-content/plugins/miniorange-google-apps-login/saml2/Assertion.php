...
class SAML2_Assertion
{
    ...
    {
        $this->id = Mo_Ga_Saml_Utility::generateId();
        $this->issueInstant = Mo_Ga_Saml_Utility::generateTimestamp();
        $this->issuer = '';
        $this->authnInstant = Mo_Ga_Saml_Utility::generateTimestamp();
		...
		if(!get_option('mo_saml_free_version') && $xml->localName === 'EncryptedAssertion'){
				$data = Mo_Ga_Saml_Utility::xpQuery($xml, './xenc:EncryptedData');
				$encryptedMethod =  Mo_Ga_Saml_Utility::xpQuery($xml, './xenc:EncryptedData/ds:KeyInfo');
				$method = $encryptedMethod[0]->firstChild->firstChild->getAttribute("Algorithm");
				$algo = Mo_Ga_Saml_Utility::getEncryptionAlgorithm($method);
				if (count($data) === 0) {
				...
				$url = plugin_dir_path(__FILE__) . 'resources' . DIRECTORY_SEPARATOR .'certs'. DIRECTORY_SEPARATOR . 'sp-key.key';
				
				$key->loadKey($url,TRUE);
				
				$alternateKey = new XMLSecurityKey($algo, array('type' => 'private'));
				
				$alternateKeyUrl = plugin_dir_path(__FILE__) . 'resources' . DIRECTORY_SEPARATOR .'certs'. DIRECTORY_SEPARATOR .'miniorange_sp_priv_key.key';
				$alternateKey->loadKey($alternateKeyUrl,TRUE);
				
				$blacklist = array();
				$xml = Mo_Ga_Saml_Utility::decryptElement($data[0], $key, $blacklist, $alternateKey);
		}
        ...
        $this->issueInstant = Mo_Ga_Saml_Utility::xsDateTimeToTimestamp($xml->getAttribute('IssueInstant'));
		
        $issuer = Mo_Ga_Saml_Utility::xpQuery($xml, './saml_assertion:Issuer');
        if (empty($issuer)) {
    ...
    {
        $subject = Mo_Ga_Saml_Utility::xpQuery($xml, './saml_assertion:Subject');
        if (empty($subject)) {
            /* No Subject node. */
        ...
        $nameId = Mo_Ga_Saml_Utility::xpQuery(
            $subject,
        ...
        if ($nameId->localName === 'EncryptedData') {
            ...
            $this->nameId = Mo_Ga_Saml_Utility::parseNameId($nameId);
        }
    }
    ...
    {
        $conditions = Mo_Ga_Saml_Utility::xpQuery($xml, './saml_assertion:Conditions');
        if (empty($conditions)) {
        ...
        if ($conditions->hasAttribute('NotBefore')) {
            $notBefore = Mo_Ga_Saml_Utility::xsDateTimeToTimestamp($conditions->getAttribute('NotBefore'));
            if ($this->notBefore === NULL || $this->notBefore < $notBefore) {
                $this->notBefore = $notBefore;
            }
        }
        if ($conditions->hasAttribute('NotOnOrAfter')) {
            $notOnOrAfter = Mo_Ga_Saml_Utility::xsDateTimeToTimestamp($conditions->getAttribute('NotOnOrAfter'));
        ...
        for ($node = $conditions->firstChild; $node !== NULL; $node = $node->nextSibling) {
            ...
            switch ($node->localName) {
                case 'AudienceRestriction':
                    $audiences = Mo_Ga_Saml_Utility::extractStrings($node, 'urn:oasis:names:tc:SAML:2.0:assertion', 'Audience');
    ...
    {
        $authnStatements = Mo_Ga_Saml_Utility::xpQuery($xml, './saml_assertion:AuthnStatement');
        ...
        $this->authnInstant = Mo_Ga_Saml_Utility::xsDateTimeToTimestamp($authnStatement->getAttribute('AuthnInstant'));

        if ($authnStatement->hasAttribute('SessionNotOnOrAfter')) {
            $this->sessionNotOnOrAfter = Mo_Ga_Saml_Utility::xsDateTimeToTimestamp($authnStatement->getAttribute('SessionNotOnOrAfter'));
        }
    ...
    private function parseAuthnContext(DOMElement $authnStatementEl)
    {
        ...
        $authnContexts = Mo_Ga_Saml_Utility::xpQuery($authnStatementEl, './saml_assertion:AuthnContext');
        ...
        $authnContextDeclRefs = Mo_Ga_Saml_Utility::xpQuery($authnContextEl, './saml_assertion:AuthnContextDeclRef');
        ...
        $authnContextDecls = Mo_Ga_Saml_Utility::xpQuery($authnContextEl, './saml_assertion:AuthnContextDecl');
        ...
        $authnContextClassRefs = Mo_Ga_Saml_Utility::xpQuery($authnContextEl, './saml_assertion:AuthnContextClassRef');
        ...
        $this->AuthenticatingAuthority = Mo_Ga_Saml_Utility::extractStrings(
            $authnContextEl,
    ...
    {
        $firstAttribute = TRUE;
        $attributes = Mo_Ga_Saml_Utility::xpQuery($xml, './saml_assertion:AttributeStatement/saml_assertion:Attribute');
        foreach ($attributes as $attribute) {
            ...
            $values = Mo_Ga_Saml_Utility::xpQuery($attribute, './saml_assertion:AttributeValue');
    ...
    {
        $this->encryptedAttribute = Mo_Ga_Saml_Utility::xpQuery(
            $xml,
    ...
    public function validate(XMLSecurityKey $key)
    {
        ...
        Mo_Ga_Saml_Utility::validateSignature($this->signatureData, $key);
     ...
     *
     * The returned NameId is in the format used by Mo_Ga_Saml_Utility::addNameId().
     *
     * @see Mo_Ga_Saml_Utility::addNameId()
     ...
     *
     * The NameId must be in the format accepted by Mo_Ga_Saml_Utility::addNameId().
     *
     * @see Mo_Ga_Saml_Utility::addNameId()
    ...
    public function encryptNameId(XMLSecurityKey $key)
    {
        ...
        Mo_Ga_Saml_Utility::addNameId($root, $this->nameId);
        $nameId = $root->firstChild;

        Mo_Ga_Saml_Utility::getContainer()->debugMessage($nameId, 'encrypt');
    ...
    public function decryptNameId(XMLSecurityKey $key, array $blacklist = array())
    {
        ...
        $nameId = Mo_Ga_Saml_Utility::decryptElement($this->encryptedNameId, $key, $blacklist);
        Mo_Ga_Saml_Utility::getContainer()->debugMessage($nameId, 'decrypt');
        $this->nameId = Mo_Ga_Saml_Utility::parseNameId($nameId);
    ...
    public function decryptAttributes(XMLSecurityKey $key, array $blacklist = array())
    {
        ...
        foreach ($attributes as $attributeEnc) {
            ...
            $attribute = Mo_Ga_Saml_Utility::decryptElement(
    ...
    public function toXML(DOMNode $parentElement = NULL)
    {
        ...
        $issuer = Mo_Ga_Saml_Utility::addString($root, 'urn:oasis:names:tc:SAML:2.0:assertion', 'saml:Issuer', $this->issuer);
        ...
        if ($this->signatureKey !== NULL) {
            Mo_Ga_Saml_Utility::insertSignature($this->signatureKey, $this->certificates, $root, $issuer->nextSibling);
        }
    ...
    private function addSubject(DOMElement $root)
    {
        ...
        if ($this->encryptedNameId === NULL) {
            Mo_Ga_Saml_Utility::addNameId($subject, $this->nameId);
        } else {
    ...
    private function addConditions(DOMElement $root)
    {
        ...
        if ($this->validAudiences !== NULL) {
            ...
            Mo_Ga_Saml_Utility::addStrings($ar, 'urn:oasis:names:tc:SAML:2.0:assertion', 'saml:Audience', FALSE, $this->validAudiences);
        }
    }
