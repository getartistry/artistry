	...
	function WC_PB_Bundle( data ) {
			...
			if ( bundled_item.has_addons() ) {
				...
				bundled_item.$bundled_item_cart
					...
					.on( 'updated_addons', function( event ) {
						...
						addons_price += bundle.get_custom_price_addons_raw_price( bundled_item );
		...
		this.get_price_html = function( price_data_array ) {
			...
			var	bundle_price_html = '',
				show_total_string = ( price_data[ 'totals' ].price !== wc_pb_number_round( price_data[ 'raw_bundle_price_min' ] ) || price_data[ 'raw_bundle_price_min' ] !== price_data[ 'raw_bundle_price_max' ] ) && false === bundle.get_bundled_subscriptions() && false === bundle.is_composited(),
		...
		this.update_addons_totals = function() {
			...
			if ( bundle.show_addons_totals ) {
				...
				if ( bundle.passes_validation() ) {
					...
					addons_raw_price += bundle.get_custom_price_addons_raw_price();

					if ( addons_raw_price > 0 ) {

						var addons_prices      = bundle.get_taxed_totals( addons_raw_price, addons_raw_price, tax_ratios, qty ),
							price_html         = '',
							price_html_suffix  = '',
							addons_totals_html = '';
						...
						addons_totals_html += '<dt>' + wc_bundle_params.i18n_addons_total + '</dt><dd><strong>' + price_html + price_html_suffix + '</strong></dd></dl>';
