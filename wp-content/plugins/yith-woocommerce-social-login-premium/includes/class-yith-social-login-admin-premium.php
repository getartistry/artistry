...
if( ! class_exists( 'YITH_WC_Social_Login_Admin_Premium' ) ){
    ...
    class YITH_WC_Social_Login_Admin_Premium extends YITH_WC_Social_Login_Admin{
        ...
        function register_panel_advanced() {
            add_action( 'woocommerce_admin_field_ywsl_social_networks_menu', array( $this, 'social_network_menu' ), 10 );
        ...
        /**
         * Show the submenu in settings page
         *
         * @return   void
         * @since    1.0.0
         * @author   Emanuela Castorina
         */
        function social_network_menu() {
            $tabs = $this->admin_tab_premium_ordered( array() );

            $current_tab = isset( $_GET['tab'] ) ? $_GET['tab'] : '';
            if ( !empty( $tabs ) ) {
                echo '<ul class="subsubsub">';
                $class = ( $current_tab == 'settings' ) ? 'class="current"' : '';
                echo '<li><a href="' . admin_url( "admin.php?page={$this->_panel_page}&tab=settings" ) . '" ' . $class . '>' . __( 'Settings', 'yith-woocommerce-social-login' ) . '</a> </li>';

                foreach ( $tabs as $key => $tab ) {
                    $class = ( $current_tab == $key ) ? 'class="current"' : '';
                    echo '<li><a href="' . admin_url( "admin.php?page={$this->_panel_page}&tab={$key}" ) . '" ' . $class . '>' . $tab . '</a> </li>';
                }
                echo '</ul>';
            }
        }
