...
class QuadMenu_LMM extends QuadMenu_Compatibility {
    ...
    function lmm($item) {
        ...
        if ($item->quadmenu == 'mega' && $columns = get_post_meta($item->ID, LMM . '_menu_item_mega_columns', true)) {

            $item->columns = explode(',', $columns);
        }

        if ($item->quadmenu == 'column' && $columns = get_post_meta($item->ID, LMM . '_menu_item_mega_sub_columns', true)) {
