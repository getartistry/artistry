...
// Add body class
$("body").addClass("upload-php-mode-" + window.rml_list_mode);
...
function rmlPrepareUpload(file) {
    ...
    var active = jQuery(".rml-root-list").find(".active"),
        opts = {};
...
function rmlUploadProgress(up, file) {
    var item = jQuery('#rml-uploading-' + file.id),
...
function rmlUploadSuccess(fileObj) {
    var item = jQuery('.rml-uploading-' + fileObj.cid);
    if (item.size() > 0) {
        var data = {
            'action': 'bulk_move',
            'ids' : [fileObj.id],
...
/**
 * Handler for the uploading method.
 */
        ...
        init : function() { // plupload 'PostInit'
            this.uploader.bind('FilesAdded', function( up, files ) {
    			for ( var i = 0 ; i < files.length ; i++ ) {
    			    rmlPrepareUpload( files[ i ] );
    			}
    		});
    		
    		this.uploader.bind('UploadProgress', function(up, file) {
    			rmlUploadProgress(up, file);
    		});
        },
        success : function( file_attachment ) { // plupload 'FileUploaded'
            rmlUploadSuccess(file_attachment);
        }
    });
}

/**
 * Handler for the main library
 */
jQuery(document).ready(function($) {
    ...
    function rmlToggleEditMode(mode) {
        if (mode) {
            oldHTML = $(".rml-container .rml-root-list").html();
    ...
    if ($("body").hasClass("wp-admin")
        ...
        /**
         * Init componennts and create the sidebar.
         */
        container = $(".rml-container.rml-dummy").clone().prependTo("body.wp-admin.upload-php #wpbody");
        container.removeClass("rml-dummy");
        root = container.find(".wrap > .list[data-root]");
        
        /**
         * Handler for clicking the button to create a new folder.
         */
        $(document).on("click", "#rml-add-new-folder", function(e) {
        ...
        /**
         * Handler for clicking the rename folder button.
         */
        $(document).on("click", "#rml-folder-rename", function(e) {
            // Check, if sub directorys are setted
            var active = $(".rml-container .rml-root-list a.active"),
            ...
            if (slug == "/" || id == -1) {
                alert($("[data-lang-rename-root]").attr("data-lang-rename-root"));
            }else{
                var name = prompt($("[data-lang-rename-prompt]").attr("data-lang-rename-prompt"), "");
                if (RMLisDefined(active[0]) && RMLisDefined(name) && name.length > 0) {
                    var cnt = active.find("span").html();
                    var newNameHTML = "<i class=\"fa fa-folder-open\"></i><i class=\"fa fa-folder\"></i>" + name + "<span class=\"rml-cnt-" + cnt + "\">" + cnt + "</span>";
                    active.html(newNameHTML);
                }
                
                if (RMLisDefined(name) && name.length > 0) {
                    $("#rml-folder-rename").find("i.fa").attr("class", "fa fa-circle-o-notch fa-spin");
                    ...
                    jQuery.post(
                        ...
                        function(response){
                            $("#rml-folder-rename").find("i.fa").attr("class", "fa fa-pencil");
                            if (response.success) {
                                $('#media-attachment-folder-filters option[value="' + id + '"]').html(name);
                            }else{
                                alert(response.data);
                                $("#rml-folder-rename").children("i.fa").attr("class", "fa fa-pencil");
                            }
                        }
                    );
                }
        ...
        /**
         * Handler for clicking the delete folder button.
         */
        $(document).on("click", "#rml-folder-delete", function(e) {
            // Check, if sub directorys are setted
            var active = $(".rml-container .rml-root-list a.active"),
                slug = active.attr("data-slug");
            if (!RMLisDefined(slug)) return;
            ...
            if (slug == "/" || id == -1) {
                alert($("[data-lang-delete-root]").attr("data-lang-delete-root"));
            }else if (active.parent().children("ul").children("li").size() > 0) {
                alert($("[data-lang-delete-failed]").attr("data-lang-delete-failed"));
            }else{
                if (window.confirm($("[data-lang-delete-confirm]").attr("data-lang-delete-confirm"))) {
                    // Change the number of items
                    var cntAll = $("#rml-list-li-all-files span").html();
                    if (RMLisDefined(cntAll)) {
                        cntAll = parseInt(cntAll);
                        var cntThis = active.find("span").html();
                        if (RMLisDefined(cntThis)) {
                            cntThis = parseInt(cntThis);
                            cntAll -= cntThis;
                            $("#rml-list-li-all-files span").html(cntAll);
                        }
                    }
                    
                    active.parent().remove();
                    $("#rml-folder-delete").find("i.fa").attr("class", "fa fa-circle-o-notch fa-spin");
                    ...
                    jQuery.post(
                        ...
                        function(response){
                            $('#media-attachment-folder-filters option[value="' + id + '"]').remove();
                            $("#rml-folder-delete").find("i.fa").attr("class", "fa fa-trash-o");
                        }
                    );
                }
            }
        ...
        /**
         * Handler for submitting the new created folder.
         */
        $(document).on("submit", "form[data-create-rml]", function(e) {
            var current = $(this);
            var name = $(this).find('input[type="text"]').val();
            if (RMLisDefined(name) && name.length > 0) {
                $(this).find("i.fa").attr("class", "fa fa-circle-o-notch fa-spin");
                var parent = $(this).attr("data-create-rml");
                ...
                jQuery.post(
                    ...
                    function(response){
                        if (response.success) {
                            ...
                            alert(response.data);
                            current.find("i.fa").attr("class", "fa fa-folder-open-o");
                        }
                    }
                );
            }else{
        ...
        /**
         * Handler for clicking the button for sort mode.
         */
        $(document).on("click", '.rml-container .view-switch-sort', function(e) {
            rmlToggleEditMode(!$(".rml-container > .wrap").hasClass("edit-mode"));
            e.preventDefault();
            return false;
        })
        
        /**
         * Click handler for buttons in sort-mode.
         * On "Cancel", show the previous folder tree.
         * On "Save", send new create ajax and refresh page.
         */
        $(document).on("click", ".rml-container .wrap.edit-mode button", function(e) {
            if ($(this).hasClass("button-primary")) {
                $(this).html('<i class="fa fa-circle-o-notch fa-spin"></i>');
                
                var arraied = $('.rml-container .rml-root-list > ul').nestedSortable('toArray', {startDepthCount: 0});
                if (RMLisDefined(arraied) && arraied.length > 1) {
                    arraied.shift();
                    var raw = [], parent;
                    $.each(arraied, function(k, v) {
                        ...
                        raw.push({fid: v.item_id, pid: v.parent_id});
                    });
                ...
                rmlToggleEditMode(false);
                $(".rml-container .rml-root-list").html(oldHTML);
            }
        });
        
        
        /**
         * Handler for clicking a tree item in sort mode.
         * ==> Do not redirect to link.
         */
        $(document).on("click", '.rml-container .wrap.edit-mode .rml-root-list a', function(e) {
            e.preventDefault();
            return false;
        });
        
        /**
         * Handler for clicking a tree item in grid mode without sorting mode.
         * When necessery ==> change select in view filter.
         */
        $(document).on("click", "body.upload-php-mode-grid .rml-container > .wrap:not(.edit-mode) a[data-id]", function(e) {
            $('body.upload-php-mode-grid .rml-container a.active').removeClass("active");
         ...
         */
        $( ".rml-root-list a" ).droppable({
            activeClass: "ui-state-default",
            hoverClass: "ui-state-hover",
            drop: function( event, ui ) {
                if ($(this).hasClass("active") || $(".rml-container > .wrap").hasClass("edit-mode")) {
                    return;
                }
                $(this).addClass("changed");
                
                var folderId = $(this).attr("data-id"), items = [], forceReload = false;
                ...
                if ($("body").hasClass("upload-php-mode-list")) {
                    ...
                    if (trs.size() > 0) {
                        trs.each(function() {
                            items.push(parseInt($(this).attr("value"), 0));
                        });
                    }else{
                        items.push(parseInt(ui.draggable.find('input[name="media[]"]').attr("value"), 0));
                    }
                }else{
                    if ($(".media-frame").hasClass("mode-select")) {
                        ...
                        items = [ ui.draggable.attr("data-id") ];
                        ui.draggable.remove();
                    }
                    
                    // Change counter from destination folder
                    var span = $(this).children("span"),
                        spanCnt = parseInt(span.html(), 0) + items.length;
                    span.html(spanCnt).attr("class", "rml-cnt-" + spanCnt);
                    
                    // Change counter from source folder
                    span = $(".rml-container .rml-root-list a.active > span");
                    spanCnt = parseInt(span.html(), 0) - items.length;
                    if (spanCnt < 0) spanCnt = 0;
                    span.html(spanCnt).attr("class", "rml-cnt-" + spanCnt);
                }
                jQuery.post(
                    window.rml_ajax_url, 
