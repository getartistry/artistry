...
class OpauthStrategy {
	 ...
	 *
	 * @param array|stdClass $profile      User profile obtained from provider
	 ...
	 * @return bool False if value cannot be mapped
	 */
	protected function mapProfile($profile, $profile_path, $auth_path) {
		...
		foreach ( $from as $element ) {
			if ( is_array( $profile ) && array_key_exists( $element, $profile ) ) {
				$value = $profile[ $element ];
			} elseif ( is_object( $profile ) && ! empty( $profile->$element ) ) {
				$value = $profile->$element;
			} else {
				return false;
			}
