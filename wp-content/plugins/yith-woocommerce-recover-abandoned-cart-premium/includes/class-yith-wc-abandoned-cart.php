    ...
    class YITH_WC_Recover_Abandoned_Cart {
        ...
        public function __construct() {
	        ...
	        if ( version_compare( WC()->version, '2.7.0', '>=' ) ) {
		        add_action( 'woocommerce_checkout_create_order', array( $this, 'remove_cart_after_order_create' ), 10, 2 );
	        } else {
		        add_action( 'woocommerce_checkout_update_order_meta', array( $this, 'remove_cart_after_order_create' ), 10, 2 );
	        }
	    ...
	    public function remove_carts_abandoned_is_expired(){
		    ...
		    $start_to_date = (int) ( current_time( 'timestamp', 1 ) - $this->delete_abandoned_time );
	     ...
	     *
	     */
        public function get_cart_content( $cart_id, $cart_content, $lang, $type = 'cart' ){
	        ...
	        if( $type == 'cart' ){
		        ...
		        wc_get_template( 'pending-order-content.php', array( 'order' => $order, 'lang' => $lang ) );
	        }
            return ob_get_clean();
        }
	    ...
	    function remove_cart_after_order_create( $order, $data ){
	    	if( is_numeric( $order ) ){
