    ...
    class YITH_WC_Recover_Abandoned_Cart_Email {
				    ...
				    if ( ! empty ( $carts ) ) {
					    foreach ( $carts as $cart ) {
						    ...
						    if ( $type == 'cart' ) {
							    ...
							    $mod_date = yit_get_prop( $cart, 'modified_date', true );
							    $mod      = get_gmt_from_date( $mod_date );

							    if ( strtotime( $mod ) < $start_to_date ) {
								    continue;
							    }
						    }
	    ...
	    public function email_send( $cart_id, $email, $lang, $type = 'cart' ) {
		   ...
		   $user_email = true;
		    ...
		    $cart_content = YITH_WC_Recover_Abandoned_Cart()->get_cart_content( $cart_id, $cart_content_meta, $lang, $type );
