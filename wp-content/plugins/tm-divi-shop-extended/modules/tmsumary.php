<?php 
class ET_Builder_Module_Sumary extends ET_Builder_Module {
	function init() {
		$this->name       = esc_html__( 'Wc Summary', 'et_builder' );
		$this->slug       = 'et_pb_sumary';
		...
		$this->advanced_options = array(
			'fonts' => array(
				'header'   => array(
					'label'    => esc_html__( 'Summary Title', 'et_builder' ),
					'css'      => array(
					...
					'text'   => array(
					'label'    => esc_html__( 'Summary Description', 'et_builder' ),
					'css'      => array(
	...
	function get_fields() {
		$fields = array(
			...
			'sku_color' => array(
				            'label'    => esc_html__( 'Text Sku Color', 'et_builder' ),
				            'type'     => 'color-alpha',
				            'custom_color'      => true,
				            'tab_slug' => 'advanced',
                    ),
				'price_color' => array(
				            'label'    => esc_html__( 'Text Price Color', 'et_builder' ),
				            'type'     => 'color-alpha',
				            'custom_color'      => true,
				            'tab_slug' => 'advanced',
                    ),
	...
	function shortcode_callback( $atts, $content = null, $function_name ) {
		...
		$price_color = $this->shortcode_atts['price_color'];
		$sku_color = $this->shortcode_atts['sku_color'];
...
if ( '' !== $sku_color ) {
			                ET_Builder_Element::set_style( $function_name, array(
				                'selector'    => '%%order_class%% span.sku_wrapper',
				                'declaration' => sprintf(
		                ...
		                if ( '' !== $price_color ) {
			                ET_Builder_Element::set_style( $function_name, array(
				                'selector'    => '%%order_class%% p.price',
				                'declaration' => sprintf(
					            'color: %1$s%2$s!important;',
					        esc_html( $price_color ),
					        et_is_builder_plugin_active() ? ' !important' : ''
				            ),
			                ) );
		                }
