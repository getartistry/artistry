 ...
 * Link Effects Widget
 */
class PP_Google_Maps_Widget extends Widget_Base {
    ...
    public function get_icon() {
        return 'fa fa-map-marker power-pack-admin-icon';
    }
    ...
    protected function render() {
        ...
        //print_r($pp_map_themes[$settings['map_theme']]);
        $i = 1;
        
        $this->add_render_attribute( 'google-map', 'id', 'pp-google-map-' . esc_attr( $this->get_id() ) );
        
        $this->add_render_attribute( 'google-map', 'class', 'pp-google-map pp-map-height' );
        
        $this->add_render_attribute( 'google-map', 'data-target', 'pp-google-map-' . esc_attr( $this->get_id() ) );

        if ( ! empty( $settings['map_zoom']['size'] ) ) {
            $this->add_render_attribute( 'google-map', 'data-zoom', $settings['map_zoom']['size'] );
        }

        if ( ! empty( $settings['map_option_streeview'] ) ) {
            $this->add_render_attribute( 'google-map', 'data-streeview-control', $settings['map_option_streeview'] );
        }

        if ( ! empty( $settings['map_type'] ) ) {
            $this->add_render_attribute( 'google-map', 'data-map-type', $settings['map_type'] );
        }

        if ( ! empty( $settings['map_type_control'] ) ) {
            $this->add_render_attribute( 'google-map', 'data-map-type-control', $settings['map_type_control'] );
        }

        if ( ! empty( $settings['zoom_control'] ) ) {
            $this->add_render_attribute( 'google-map', 'data-zoom-control', $settings['zoom_control'] );
        }

        if ( ! empty( $settings['fullscreen_control'] ) ) {
            $this->add_render_attribute( 'google-map', 'data-fullscreen-control', $settings['fullscreen_control'] );
        }

        if ( $settings['map_scroll_zoom'] == 'yes' ) {
            $this->add_render_attribute( 'google-map', 'data-scroll-zoom', $settings['map_scroll_zoom'] );
        }

        if ( $settings['map_theme'] != 'standard' ) {
            if ( $settings['map_theme'] != 'custom' ) {
                $this->add_render_attribute( 'google-map', 'data-custom-style', $pp_map_themes[$settings['map_theme']] );
        ...
        foreach ( $settings['pp_map_addresses'] as $index => $item ) {
            $pin_key = 'pin_' . $i;
            $pp_map_locations .= '["' . $item['map_latitude'] . '",';
            $pp_map_locations .= '"' . $item['map_longitude'] . '",';
            $pp_map_locations .= '"';
            if ( $item['map_marker_infowindow'] == 'yes' ) {
                $pp_map_locations .= 'yes';
            }
            $pp_map_locations .= '",';
            $pp_map_locations .= '"' . $item['map_title'] . '",';
            $pp_map_locations .= '"' . $item['map_description'] . '",';
            if ( $item['map_marker_icon_type'] == 'custom' && $item['map_marker_icon']['url'] != '' ) {
                $pp_map_locations .= '"custom","' . $item['map_marker_icon']['url'] . '"';
            } else {
                $pp_map_locations .= '"", ""';
            }
            $pp_map_locations .= ',' . '"';
            if ( $item['map_marker_icon_type'] == 'custom' ) {
                $pp_map_locations .= $item['map_custom_marker_size'];
            }
            $pp_map_locations .= '"';
            $pp_map_locations .= ',' . '"';
            if ( $item['map_info_window_open'] == 'yes' ) {
                $pp_map_locations .= 'iw_open';
            }
            $pp_map_locations .= '"';
            $pp_map_locations .= ']';
            if ( $index != count( $settings['pp_map_addresses'] ) -1 ) { $pp_map_locations .= ','; }
        }
        $pp_map_locations .= ']';
        
        //echo $pp_map_locations;
	?>
    <div class="pp-google-map-container">
        <div <?php echo $this->get_render_attribute_string( 'google-map' ); ?> data-locations='<?php echo esc_attr( $pp_map_locations ); ?>'></div>
    </div>
	<?php
	}
...
Plugin::instance()->widgets_manager->register_widget_type( new PP_Google_Maps_Widget() );