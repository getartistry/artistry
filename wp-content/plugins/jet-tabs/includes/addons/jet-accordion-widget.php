...
class Jet_Accordion_Widget extends Jet_Tabs_Base {
	...
	protected function render() {
		...
		$toggles = $this->get_settings( 'toggles' );
				...
				<?php
					foreach ( $toggles as $index => $item ) {
						...
						if ( ! empty( $item['item_icon'] ) && ! empty( $item['item_active_icon'] ) ) {
							$toggle_control_icon_html .= sprintf( '<div class="jet-toggle__label-icon"><i class="jet-toggle__icon icon-normal %1$s"></i><i class="jet-toggle__icon icon-active %2$s"></i></div>', $item['item_icon'], $item['item_active_icon'] );
						}
						...
						if ( '0' !== $item['item_template_id'] ) {
							...
							$content_html .= jet_tabs()->elementor()->frontend->get_builder_content_for_display( $item['item_template_id'] );
							$link = add_query_arg(
								array(
									'elementor'       => '',
									'jet-tabs-canvas' => '',
							...
							if ( jet_tabs_integration()->in_elementor() ) {
								$content_html .= '<div class="jet-toggle__edit-cover" data-template-edit-link="' . $link . '"><i class="fa fa-pencil"></i></div>';
							}
						} else {
	...
	public function no_templates_message() {
		...
		$new_link = '<span>' . esc_html__( 'Select an existing template or create a ', 'jet-tabs' ) . '</span><a class="jet-toogle-new-template-link elementor-clickable" target="_blank" href="' . Utils::get_create_new_post_url( 'elementor_library' ) . '">' . esc_html__( 'new one', 'jet-tabs' ) . '</a>' ;
