...
class Jet_Tabs_Widget extends Jet_Tabs_Base {
			...
			<div class="jet-tabs__control-wrapper">
				<?php
					foreach ( $tabs as $index => $item ) {
						...
						echo sprintf( '<div %1$s><div class="jet-tabs__control-inner">%2$s%3$s</div></div>', $this->get_render_attribute_string( $tab_title_setting_key ), $title_icon_html , $title_label_html );
					}
				?>
			</div>
					...
					foreach ( $tabs as $index => $item ) {
						...
						if ( '0' !== $item['item_template_id'] ) {
							...
							if ( jet_tabs_integration()->in_elementor() ) {
								$content_html .= '<div class="jet-tabs__edit-cover" data-template-edit-link="' . $link . '"><i class="fa fa-pencil"></i></div>';
							}
						} else {
	...
	public function no_templates_message() {
		...
		$new_link = '<span>' . esc_html__( 'Select an existing template or create a ', 'jet-tabs' ) . '</span><a class="jet-tabs-new-template-link elementor-clickable" target="_blank" href="' . Utils::get_create_new_post_url( 'elementor_library' ) . '">' . esc_html__( 'new one', 'jet-tabs' ) . '</a>' ;
